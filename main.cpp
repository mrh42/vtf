...
//
const uint64_t M = 4 * 3L * 5 * 7 * 11 * 13 * 17 * 19 * 23;  // 446,185,740
const uint64_t M2 = 29 * 31 * 37 * 41 * 43;
const uint32_t ListLen = 72990720;
...
struct Stuff {
	...
	uint    Ll;           // ListLen, when L >= Ll, threads will return.
        uint    List[ListLen];
	uint    KmodM2;
	uint    X2[M2];
...
struct Stuff2 {
	uint       Listx[ListLen];  // copy of List.  Just a tiny speed up.
	uint    X2x[M2];
};
